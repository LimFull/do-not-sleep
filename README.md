...
> **업데이트 후 마우스가 움직이지 않는 경우**: ad-hoc 서명은 빌드마다
> 바뀌므로 앱을 새 버전으로 교체하면 기존 접근성 권한이 무효가 됩니다.
> 시스템 설정에는 허용된 것으로 보여도 실제로는 거부 상태입니다.
> 아래 명령으로 권한을 초기화한 뒤 앱을 재실행해 다시 허용하세요.
>
> ```sh
> tccutil reset Accessibility local.donotsleep.menubar
> ```
